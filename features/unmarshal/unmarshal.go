...
func (p *unmarshal) fieldItem(field *protogen.Field, fieldname string, message *protogen.Message, proto3 bool) {
	...
	case protoreflect.DoubleKind:
		...
		if oneof {
			p.P(`m.`, fieldname, ` = `, field.Oneof.GoIdent, `{ value: `,
				p.Ident("github.com/bogdandrutu/lazyproto/variant", "Float64Value"),
				`(`, p.Ident("math", `Float64frombits`), `(v)), valueType: `,
				p.OneofFieldTypeFromField(field), `}`)
		} else if repeated {
	...
	case protoreflect.FloatKind:
		...
		if oneof {
			p.P(`m.`, fieldname, ` = `, field.Oneof.GoIdent, `{ value: `,
				p.Ident("github.com/bogdandrutu/lazyproto/variant", "Float64Value"),
				`(`, p.Ident("math", `Float32frombits`), `(v)), valueType: `,
				p.OneofFieldTypeFromField(field), `}`)
		} else if repeated {
	...
	case protoreflect.Int64Kind:
		if oneof {
			...
			p.P(`m.`, fieldname, ` = `, field.Oneof.GoIdent, `{ value: `,
				p.Ident("github.com/bogdandrutu/lazyproto/variant", "Int64Value"),
				`(v), valueType: `, p.OneofFieldTypeFromField(field), `}`)
		} else if repeated {
			p.P(`var v `, typ)
			p.decodeVarint("v", typ)
	...
	case protoreflect.BoolKind:
		...
		if oneof {
			p.P(`b := `, typ, `(v != 0)`)
			p.P(`m.`, fieldname, ` = `, field.Oneof.GoIdent, `{ value: `,
				p.Ident("github.com/bogdandrutu/lazyproto/variant", "BoolValue"),
				`(b), valueType: `, p.OneofFieldTypeFromField(field), `}`)
		} else if repeated {
	...
	case protoreflect.StringKind:
		...
		if oneof {
			p.P(`m.`, fieldname, ` = `, field.Oneof.GoIdent, `{ value: `,
				p.Ident("github.com/bogdandrutu/lazyproto/variant", "StringValue"),
				`(string(dAtA[iNdEx:postIndex]))`, `, valueType: `, p.OneofFieldTypeFromField(field), `}`)
		} else if repeated {
	...
	case protoreflect.MessageKind:
		...
		if oneof {
			...
			p.P(`m.`, fieldname, ` = `, field.Oneof.GoIdent, `{ value: `,
				p.Ident("github.com/bogdandrutu/lazyproto/variant", "PointerValue"),
				`(`, p.Ident("unsafe", "Pointer"), `(v)), valueType: `, p.OneofFieldTypeFromField(field), `}`)
			...
			// TODO: here needs a condition based on nullable
	...
	case protoreflect.BytesKind:
		...
		if oneof {
			...
			p.P(`m.`, fieldname, ` = `, field.Oneof.GoIdent, `{ value: `,
				p.Ident("github.com/bogdandrutu/lazyproto/variant", "BytesValue"),
				`(v), valueType: `, p.OneofFieldTypeFromField(field), `}`)
		} else if repeated {
...
func (p *unmarshal) field(oneof bool, field *protogen.Field, message *protogen.Message) {
	...
	if field.Desc.IsList() && wireType != protowire.BytesType {
		...
		p.P(`return `, p.Ident("fmt", "Errorf"), `("proto: wrong wireType = %d for field `, errFieldname, `", wireType)`)
		p.P(`}`)
		p.fieldItem(field, fieldname, message, true)
	}
}
