...
func (p *marshal) mapField(kvField *protogen.Field, varName string) {
	...
	case protoreflect.MessageKind:
		p.marshalBackward(varName, true, p.IsLocalMessage(kvField.Message))
	}
}
...
func (p *marshal) field(oneof bool, numGen *counter, field *protogen.Field) {
	...
	case protoreflect.DoubleKind:
		if packed {
			...
			p.encodeFixed64(p.Ident("math", "Float64bits"), `(float64(m.value.AsFloat64()))`)
			p.encodeKey(fieldNumber, wireType)
		} else {
	...
	case protoreflect.FloatKind:
		if packed {
			...
			p.encodeFixed32(p.Ident("math", "Float32bits"), `(float32(m.value.AsFloat64()))`)
			p.encodeKey(fieldNumber, wireType)
		} else {
			p.P(`if m.`, fieldname, ` != 0 {`)
			p.encodeFixed32(p.Ident("math", "Float32bits"), `(float32(m.`+fieldname, `))`)
	...
	case protoreflect.Int64Kind, protoreflect.Uint64Kind, protoreflect.Int32Kind, protoreflect.Uint32Kind, protoreflect.EnumKind:
		if packed {
			...
			p.encodeVarint(`m.value.AsInt64()`)
			p.encodeKey(fieldNumber, wireType)
		} else {
			p.P(`if m.`, fieldname, ` != 0 {`)
			p.encodeVarint(`m.`, fieldname)
	...
	case protoreflect.StringKind:
		if repeated {
			...
			p.P(`i -= len(m.value.AsString())`)
			p.P(`copy(dAtA[i:], m.value.AsString())`)
			p.encodeVarint(`len(m.value.AsString())`)
			p.encodeKey(fieldNumber, wireType)
		} else {
			p.P(`if len(m.`, fieldname, `) > 0 {`)
			p.P(`i -= len(m.`, fieldname, `)`)
	...
	case protoreflect.GroupKind:
		...
		p.marshalBackward(`m.`+fieldname, false, p.IsLocalMessage(field.Message))
	...
	case protoreflect.MessageKind:
		if field.Desc.IsMap() {
			...
			val := p.reverseListRange(`m.`, fieldname)
			p.marshalBackward(val, true, p.IsLocalMessage(field.Message))
			p.encodeKey(fieldNumber, wireType)
			p.P(`}`)
		} else if oneof {
			p.marshalBackward(`(*`+p.QualifiedGoIdent(field.Message.GoIdent)+`)(m.value.AsPointer())`, true, p.IsLocalMessage(field.Message))
			p.encodeKey(fieldNumber, wireType)
		} else {
			p.marshalBackward(`m.`+fieldname, true, p.IsLocalMessage(field.Message))
			p.encodeKey(fieldNumber, wireType)
		}
	case protoreflect.BytesKind:
		if repeated {
			...
			p.P(`i -= len(m.value.AsBytes())`)
			p.P(`copy(dAtA[i:], m.value.AsBytes())`)
			p.encodeVarint(`len(m.value.AsBytes())`)
			p.encodeKey(fieldNumber, wireType)
		} else {
			p.P(`if len(m.`, fieldname, `) > 0 {`)
			p.P(`i -= len(m.`, fieldname, `)`)
...
func (p *marshal) message(message *protogen.Message) {
	...
	for i := len(message.Fields) - 1; i >= 0; i-- {
		...
		if !oneof {
			...
			if _, ok := oneofs[fieldname]; !ok {
				...
				// TODO: Investigate why is needed to marshal forward.
				p.marshalBackward(`m.`+fieldname, false, true)
			}
		}
	}
	...
	for _, oneof := range message.Oneofs {
		...
		p.P(`func (m *`, oneof.GoIdent, `) MarshalToVT(dAtA []byte) (int, error) {`)
		p.P(`size := m.SizeVT()`)
		p.P(`return m.MarshalToSizedBufferVT(dAtA[:size])`)
		p.P(`}`)
		p.P(``)
		p.P(`func (m *`, oneof.GoIdent, `) MarshalToSizedBufferVT(dAtA []byte) (int, error) {`)
		p.P(`i := len(dAtA)`)
		p.P(`_ = i`)
		p.P(`switch m.valueType {`)
		for i := range oneof.Fields {
			p.P(`case `, p.OneofFieldType(oneof, i), `:`)
			p.field(true, &numGen, oneof.Fields[i])
		}
		p.P(`}`)
		p.P(`return len(dAtA) - i, nil`)
		p.P(`}`)
...
func (p *marshal) marshalBackward(varName string, varInt bool, local bool) {
	if local {
